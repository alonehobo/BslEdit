...
- `preview(operation, …)` — работа с открытым превью одним инструментом:
  ...
  `select`, `scroll`, `reload`, `url` (адрес для клиента со своим браузером),
  `close` и операции с аннотациями (ниже).
- `open_preview` с `base_path` или `base_revision` (`HEAD`, ветка, тег, коммит,
  `index`) показывает сравнение форм, макетов и СКД: счётчик изменений,
  переход к следующему/предыдущему (F7), фильтры и просмотр XML.
...
Пользователь оставляет в превью формы, макета или MXL пометки к элементам и
ячейкам: щелчок выбирает элемент, карандаш рядом с курсором или `Alt+A`
открывают редактор пометки; у элемента с открытой пометкой они правят её, а не
заводят вторую. Щелчок по пометке в списке или по её номеру на форме выделяет
элемент и открывает его вкладку. Список пометок перетаскивается за
заголовок и запоминает место. Ячейка называется `R1C1`, именованная область —
своим именем.

Агент работает с ними через `preview`:

- `annotations` — открытые пометки пользователя (`include_resolved` — и решённые);
- `resolve_annotation` — отметить пометку решённой с кратким `resolution`;
  пользователь принимает решение или открывает пометку снова;
- `add_annotation` — оставить пометку пользователю на `element_id` (ячейка
  `rNcM`, диапазон — через `to_element_id`);
- `remove_annotation` — удалить только свою пометку.

Пометки хранятся в сессии сервера и переживают `reload` и правку файла, но в
`Form.xml`/`Template.xml` не пишутся. При запуске с `--editor` превью
открывается в BSLEdit, и пометки, оставленные там, видны агенту так же.
